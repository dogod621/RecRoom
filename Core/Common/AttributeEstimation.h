...
namespace RecRoom
{
	...
	public:
		SurfaceEstimation(const CONST_PTR(ScannerPc)& scanner,
			...
			: ThreadAble(), scanner(scanner), cutFalloff(cutFalloff), minRequireNumData(minRequireNumData)
		{
	...
	public:
		AttributeEstimation(const CONST_PTR(ScannerPc)& scanner,
			...
			const int minRequireNumData = 1)
			: SurfaceEstimation<InPointType, OutPointType>(scanner, cutFalloff, minRequireNumData), cutGrazing(cutGrazing)
		{
